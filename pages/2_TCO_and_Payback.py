...
with slider_col:
    ...
    # Apply any pending suggestion *before* the slider widget below is
    # instantiated. Streamlit forbids writing to a widget's own session_state
    # key after that widget has already been created in the same run, so the
    # button below stages its value in "monthly_additive_pending" instead and
    # reruns — this block then applies it here, ahead of the widget.
    if "monthly_additive_pending" in st.session_state:
        st.session_state["monthly_additive_val"] = st.session_state.pop("monthly_additive_pending")
    ...
    if chosen_vehicle != "— none —":
        ...
        if st.button("Apply suggested value"):
            st.session_state["monthly_additive_pending"] = suggested_additive
            st.rerun()
